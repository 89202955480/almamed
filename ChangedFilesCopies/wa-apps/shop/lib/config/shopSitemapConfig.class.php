...
class shopSitemapConfig extends waSitemapConfig
{
    ...
    public function execute($n = 1)
    {
        ...
        foreach ($routes as $route) {
            ...
            if ($n == 1) {
                // categories
                //Стандартный запрос, который не выбирает скрытые категории
...
//                        ORDER BY c.left_key";
                //новый запрос, который выбирает в том числе и скрытые категории
                $sql = "SELECT c.id,c.parent_id,c.left_key,c.url,c.full_url,c.create_datetime,c.edit_datetime
                        FROM shop_category c
                        LEFT JOIN shop_category_routes cr ON c.id = cr.category_id
                        WHERE (cr.route IS NULL OR cr.route = '".$category_model->escape($route_url)."')
                        ORDER BY c.left_key";
