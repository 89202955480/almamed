<?php
	...
	//WEBSITE URL
	$website_url = str_replace("/backupfilemaster" , "" , ROOT_URL);
	...
	function fxn_send($to, $subject, $body, $from = "", $file = false) {
		global $settings;

		if(preg_match("/.*?<\s*(.*?)\s*>/i", html_entity_decode($to, ENT_QUOTES), $match)) {
			$to = $match[1];
		}
	 ...
	 */
	function debug($arg) {
		echo "<pre>".htmlentities(print_r($arg, true), ENT_QUOTES, "UTF-8")."</pre>";
	}
