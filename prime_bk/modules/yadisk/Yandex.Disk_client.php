...
class webdav_client {
 ...
 /**
	* Public method public_file
	*
	* Publishes the file to Yandeks.Disk
	*
	* @param string path
	* @return int status-code read from webdavserver. False on error.
	*/
 ...
 /**
	* Public method private_file
	*
	* Closes access to the link to the file in Yandeks.Disk
	*
	* @param string path
	...
	function _process_respond() {
		...
		for($i=1; $i<count($lines); $i++) {
			...
			if (!$header_done ) {
				...
				if (!isset($ret_struct['header'][$fieldname]) || ! $ret_struct['header'][$fieldname]) {
