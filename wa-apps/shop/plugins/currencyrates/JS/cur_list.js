$(function(){
 var setting_fields = "<fieldset class='setting_fields'>"+
					  ...
					  "<div class='setting_div'><h3>Множитель</h3>"+
					  "<p>$cur_rate - курс валюты на последнюю дату обновления</p>"+
					  "<p>$cur_rate * <input type='text' class='multiple' size='5'>+<input type='text' class='summary' size='5'></p></div>"+
					  "<div class='setting_div'><h3>Округление числа</h3>"+
					  ...
					  "<input type='radio' name='rounding_side'  class='rounding_side' value='2'>В ближайшую сторону</div><br/><br/>"+
					  "<input type='button' class='button blue' value='Просмотр' id='preview'><span class='state'></span>"+
                     "</fieldset>";
					 
var close_settings = '<div class="close_settings"><a href="javascript:void(0);" class="button">Скрыть настройки</a></div>';
var css = {
...
//Обработка клика по настройкам					 
 $('#cur_list').find('tr').find('.settings').click(function(){
   $(this).unbind('click');
   var code_data = $(this).parent().parent().parent().data('cur');
   showSettings(code_data);
 });

//Обновить для определённной валюты	 
 function  UpdateRate(){
  var cur = $(this).parent().parent().data('cur');
  var self = $(this);
  var data = new Array();
  data.push(cur);
  ...
  $.post('?plugin=currencyrates&module=update',{code: data},function(response){
   ...
   {
    var update_all_flag = true; //Флаг для кнопки "Обновить курс всех валют"
 ...
 function UpdateAllRates(){
  ...
  $.post('?plugin=currencyrates&module=update', {code: data}, function(response){
   ...
   {
     if(response.data.state[data[i]])//Если есть курс для этой валюты
	 {
...
function showSettings(code_data)
{
 var reverse_flag = false;
  ...
  $.post('?plugin=currencyrates&module=getrate',{code:code_data},function(response){
   ...
   {
    $('#cur_list').find('tr[data-cur="'+code_data+'"]').find('td:eq('+1+')').append(setting_fields);//Выводим панель настроек
	//Вставляем дату
	$('#cur_list').find('tr[data-cur="'+code_data+'"]').find('td:eq('+1+')').find('.setting_fields').find('h3:first').find('.date').text(response.data.date);
	if(response.data.date == null)//Если даты нет
	{
...
function RoundingTemplate(data, code_data)
{
 switch(data)
 {
  case 1: $('#cur_list').find('tr[data-cur="'+code_data+'"]').find('.setting_fields').find('.rounding_template').text('ss');
  break;
		 
  case 2: $('#cur_list').find('tr[data-cur="'+code_data+'"]').find('.setting_fields').find('.rounding_template').text('ss,s');
  break;
		 
  case 3: $('#cur_list').find('tr[data-cur="'+code_data+'"]').find('.setting_fields').find('.rounding_template').text('ss,ss');
  break;
		 
  default: $('#cur_list').find('tr[data-cur="'+code_data+'"]').find('.setting_fields').find('.rounding_template').text('');
  break;		 
 }
}
